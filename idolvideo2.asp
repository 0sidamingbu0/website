...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="810" height="50" align="left"background="images/sb_bg2.gif">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="daohang_lj"><a href="index.asp">首页</a> &gt; <%if rs("leibie")="电视" then%>
    <a href="news.asp">新闻</a>
    <%elseif rs("leibie")="作品" then%>
    <a href="production.asp">作品</a>
    <%elseif rs("leibie")="公告" then%>
    <a href="bulletin.asp">公告</a>
    <%end if%> &gt; <%=rs("mc")%> &gt; <%if session("islogin")="yes" then%>
...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  session(rs("id"))="yes"
	  %>
        <td width="340" height="22"><span class="daohang_lj" style="font-size:14px">视频：<%=rs("mc")%></span></td>
        <td width="134"></td>
        <td width="121"></td>
        <td width="5"></td>
	...
	%>
    <form id="form5" name="form5" method="post" action="video.asp">
          <div align="right"><input type="submit" name="button2" id="button2" value="删除" /><input type="hidden" name="dell" id="dell" value="<%=rs("id")%>" /></div>
          </form>
    
        <form id="form3" name="form3" method="post" action="idolvideo2.asp?id=<%=request("id")%>">
          <table width="100%" height="241" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td width="25" height="241" valign="top">&nbsp;</td>
              <td width="334" valign="top"><p class="daohang_lj">id=<%=rs("id")%>&nbsp;&nbsp;mc=
          <input name="mc" type="text" id="mc" value="<%=rs("mc")%>" size="20" maxlength="300" />
          &nbsp;&nbsp;mce=
          <input name="mce" type="text" id="mce" value="<%=rs("mce")%>" size="7" maxlength="300" /></p>
                <p class="daohang_lj">点击：<input name="djs" type="text" id="djs" value="<%=rs("djs")%>" size="8" maxlength="30" />
                  &nbsp;&nbsp;&nbsp;发布：
          <input name="date" type="text" id="date" value="<%=rs("date2")%>" size="15" maxlength="300" /></p>
                <p class="daohang_lj">&nbsp;&nbsp;来源：<input name="from2" type="text" id="from2" value="<%=rs("from2")%>" size="7" maxlength="300" />&nbsp;&nbsp;类别：<input name="leibie" type="text" id="leibie" value="<%=rs("leibie")%>" size="7" maxlength="30" />&nbsp;&nbsp;tps=<input name="tps" type="text" id="tps" value="<%=rs("tps")%>" size="3" maxlength="30" /></p>
                <p class="daohang_lj">&nbsp;&nbsp;flash url=<input name="flvurl2" type="text" id="flvurl2" value="<%=rs("flvurl2")%>" size="30" maxlength="300" /></p>
                <p class="daohang_lj">&nbsp;&nbsp;&nbsp;&nbsp;
                  <textarea name="neirong" cols="40" rows="15" id="neirong"><%=rs("neirong")%></textarea>
  ...
  <tr>
    ...
    <td width="180" height="140"><table width="160" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        ...
        <img onclick="location.href='images.asp?im=<%=rs("tps")%>&id=<%=id%>&scr=idolvideo2'" src="upload/<%=rs("mce")%>/<%=rs("tps")%>_160120.jpg" height="120" alt="" />
        <%else%>
        <img onclick="location.href='images.asp?im=<%=im-1%>&id=<%=id%>&scr=idolvideo2'" src="upload/<%=rs("mce")%>/<%=im-1%>_160120.jpg" height="120" alt="" />
        <%end if%></td>
      </tr>
    </table></td>
    <td width="180"><table width="160" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td width="160" height="120" align="center"><img onclick="location.href='images.asp?im=<%=im%>&id=<%=id%>&scr=idolvideo2'" src="upload/<%=rs("mce")%>/<%=im%>_160120.jpg" height="120" alt="" /></td>
      </tr>
    </table></td>
    <td width="180"><table width="160" border="0" align="center" cellpadding="0" cellspacing="0">
      <tr>
        <td width="160" height="120" align="center"><%if im-rs("tps")=0 then %>
          <img onclick="location.href='images.asp?im=<%=im+1%>&id=<%=id%>&scr=idolvideo2'" src="upload/<%=rs("mce")%>/<%=im+1%>_160120.jpg" height="120" alt="" /><img onclick="location.href='images.asp?im=1&id=<%=id%>&scr=idolvideo2'" src="upload/<%=rs("mce")%>/1_160120.jpg" height="120" alt="" />
        <%else%>
        <%end if%></td>
      </tr>
    </table></td>
