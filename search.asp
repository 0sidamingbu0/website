...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="810" height="50" align="left"background="images/sb_bg2.gif">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="daohang_lj"><a href="index.asp">首页</a> &gt; 搜索 &gt; “<%=search_txt%>” &gt; <%if session("islogin")="yes" then%>
    <a class="daohang_lj" href="?se=1&search_txt=<%=Request("search_txt")%>"><%se=request("se")
...
%>
  ...
  <tr>
	...
	<%if a=1 then 
	exec="select * from config where mc like '%"&search_txt&"%'  order by date2 DESC"%>
	<%else 
	exec="select * from config where mc like '%"&search_txt&"%'  order by djs DESC"%>
	<%end if%>
...
if rs.eof or rs.bof then %>
    <div align="center" class="daohang_lj">  没有找到符合"<%=search_txt%>"的结果</div>
    <%else%>
...
%>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td height="25" colspan="2" bgcolor="#E4EAF2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="370" height="25" class="daohang_lj" style="font-size:14px">
          
            
          <%if rs("leibie")="公告" then %>
          
          <%if session("islogin")="yes" then%>
             <a href="idolbulletin2.asp?id=<%=rs("id")%>"><%=rs("mc")%></a>
            <%else%>
             <a href="bulletin2.asp?id=<%=rs("id")%>"><%=rs("mc")%></a>
   			<%end if%>
          
          <%end if%>
          <%if rs("leibie")="报纸" or  rs("leibie")="杂志" then %>
          
          <%if session("islogin")="yes" then%>
             <a href="idolnews2.asp?id=<%=rs("id")%>"><%=rs("mc")%></a>
            <%else%>
             <a href="news2.asp?id=<%=rs("id")%>"><%=rs("mc")%></a>
   			<%end if%>
           
           <%end if%>
         <%if rs("leibie")="作品" or  rs("leibie")="电视" then %>
         
         <%if session("islogin")="yes" then%>
             <a href="idolvideo2.asp?id=<%=rs("id")%>"><%=rs("mc")%></a>
            <%else%>
             <a href="video2.asp?id=<%=rs("id")%>"><%=rs("mc")%></a>
   			<%end if%>
           
            <%end if%>
            </td>
            <td width="300" height="25" align="right" class="daohang_lj">【<%=rs("date2")%>】</td>
          </tr>
        </table></td>
        </tr>
      <tr>
        <td width="160" height="20">&nbsp;</td>
        <td width="534" height="130" rowspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td height="20" colspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td width="32%">&nbsp;</td>
                <td width="33%" class="daohang_lj">来源：<%=rs("from2")%></td>
                <td width="35%"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <td width="51%" class="daohang_lj">类别：<%=rs("leibie")%></td>
                    <td width="49%" class="daohang_lj">点击：<%=rs("djs")%></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
          ...
          <tr>
            ...
            <td width="518" height="120" class="daohang_lj"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%=neirong%> 
            <%if rs("leibie")="公告" then %>
            
            <%if session("islogin")="yes" then%>
             <a href="idolbulletin2.asp?id=<%=rs("id")%>">【详细介绍】</a>
            <%else%>
             <a href="bulletin2.asp?id=<%=rs("id")%>">【详细介绍】</a>
   			<%end if%>
          ...
          <%if session("islogin")="yes" then%>
             <a href="idolnews2.asp?id=<%=rs("id")%>">【详细介绍】</a>
            <%else%>
             <a href="news2.asp?id=<%=rs("id")%>">【详细介绍】</a>
   			<%end if%>
         ...
         <%if session("islogin")="yes" then%>
             <a href="video2.asp?id=<%=rs("id")%>">【详细介绍】</a>
            <%else%>
            <a href="video2.asp?id=<%=rs("id")%>">【详细介绍】</a>
   			<%end if%>
      ...
      <tr>
        <td width="160" height="120" align="center"><img onclick="location.href='images.asp?id=<%=rs("id")%>&curpage=<%=curpage%>&scr=search&search_txt=<%=search_txt%>'" name="" src="upload/<%=rs("mce")%>/1_160120.jpg" height="120" alt="" /></td>
      </tr>
      <tr>
        <td height="20" colspan="2" class="daohang_lj" style="font-size:9px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;（点击图片放大）</td>
        </tr>
    </table>
    ...
    <div align="right"><a  class="daohang_lj"  href="?a=1">发布</a>/<a  class="daohang_lj"  href="?a=2">关注</a></div>
