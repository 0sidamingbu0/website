...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <p>&nbsp;</p>
     <%if yz=yz2 then %>
    <%if bt="" or nr="" then%>
    <p style="color:#F00" class="daohang_lj">标题或内容不能为空！！ <a class="daohang_lj" href="bulletin2.asp?id=<%=request("id")%>">返回</a></p>
    <%else%>
        <%if session("islogin")="yes" then%>
            <meta  http-equiv=refresh  content=2;URL="idolcomment.asp?id=<%=request("id")%>">
            <%else%>
            <meta  http-equiv=refresh  content=2;URL="comment.asp?id=<%=request("id")%>">
   			<%end if%> 
        <%
    exec66="INSERT INTO comment (mc,suoshu,ip,date2,neirong) VALUES ('"&bt&"',"&id8&",'"&ip&"','"&now&"','"&nr&"')"
'exec="insert into comment(mc,neirong,ip)values('"+bt+"',"+nr+",'"+ip+"')" 
 '（假设name="aaa",tel=111,message="bbb"）："insert into guestbook(name,tel,message)values('aaa',111,'bbb')"。
conn.execute exec66
%>
    ...
    <%if session("islogin")="yes" then%>
            <a href="idolcomment.asp?id=<%=request("id")%>">显示全部</a>
            <%else%>
            <a href="comment.asp?id=<%=request("id")%>">显示全部</a>
   			<%end if%> 
    
    </p>
    <%end if%>
    <%else%>
     <p style="color:#F00" class="daohang_lj">验证错误！！ <a class="daohang_lj" href="bulletin2.asp?id=<%=id9%>">返回</a></p>
    <%end if%>
  </tr>
</table>
<%
