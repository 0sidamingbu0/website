...
<%a=request("a")%>
	...
	<%if a=1 then 
	exec="select * from config where leibie='公告' order by date2 DESC"%>
	<%else 
	exec="select * from config where leibie='公告' order by djs DESC" %>
	<%end if%>
...
<table width="960" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <td width="810" height="50" align="left"background="images/sb_bg2.gif">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="daohang_lj"><a href="index.asp">首页</a> &gt; 公告 &gt; <%if session("islogin")="yes" then%>
...
%>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td height="25" colspan="2" bgcolor="#E4EAF2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <%if session("islogin")="yes" then%>
            <a href="idolbulletin2.asp?id=<%=rs("id")%>&scr=bulletin&curpage=<%=curpage%>"><%=rs("mc")%></a>
            <%else%>
            <a href="bulletin2.asp?id=<%=rs("id")%>&scr=bulletin&curpage=<%=curpage%>"><%=rs("mc")%></a>
   			<%end if%>           
 </td>
      ...
      <tr>
        ...
        <td width="534" height="130" rowspan="2"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            ...
            <%if session("islogin")="yes" then%>
            <a href="idolbulletin2.asp?id=<%=rs("id")%>&scr=bulletin&curpage=<%=curpage%>">【详细介绍】</a>
            <%else%>
            <a href="bulletin2.asp?id=<%=rs("id")%>&scr=bulletin&curpage=<%=curpage%>">【详细介绍】</a>
   			<%end if%>
      ...
      <tr>
        <td width="160" height="120" align="center"><img onclick="location.href='images.asp?id=<%=rs("id")%>&scr=bulletin&curpage=<%=curpage%>'" src="upload/<%=rs("mce")%>/1_160120.jpg" height="120" /></td>
      </tr>
      <tr>
      ...
      <%rs.movenext
	next%>
    <div align="right"><a  class="daohang_lj"  href="?a=1&scr=bulletin&curpage=<%=curpage%>">发布</a>/<a  class="daohang_lj"  href="?a=2&scr=bulletin&curpage=<%=curpage%>">关注</a></div>
